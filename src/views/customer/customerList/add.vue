  ...
  <my-dialog :visible="dialogVisible"
    ...
    <el-form ref="ruleForm"
      ...
      <div class=""
        ...
        <el-form-item label="手机号(默认登录账号)"
                      prop="telephone">
          <el-input :disabled="option == 'edit'"
      ...
      <div class=""
        ...
        <el-form-item v-for="(item, index) in qualificationsList"
          ...
          <div class="flex-box">
            <!--:id="(getType == 1 || option == 'add' || channel == 'app') ? item.id : item.parameterId"-->
            <upload :title="item.name"
                    :max="item.max"
                    :isMust="item.isMust"
                    :id="item.parameterId"
                    :eventDetail.sync="item.eventDetail">
            </upload>
          </div>
        </el-form-item>
...
import { clueDetail, clueSwitch } from "@/api/clue";
export default {
  props: {
    showDialog: {
      required: true,
    ...
    return {
      ...
      rules: {
        ...
        taxpayerIdentificationNumber: [
          ...
          { min: 18, max: 32, message: '请输入正确的企业营业执照', trigger: 'blur' },
        ],
        ...
        eventDetail: [
          { required: true, message: "请选择必填资质", trigger: "blur" },
        ],
  ...
  methods: {
      ...
      detail(data)
        .then((res) => {
          if (res.resp_code == 0) {
            ...
            this.formData.areaId = this.formData.areaId || '0000'//地址id回显
            this.channel = res.data.mobile != undefined ? 'app' : '' //app是康药多注册客户
    ...
    save () {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          // 地址选择错误
          if (this.formData.areaId == '0000') {
            this.$message({ message: "经营地址有误，请重新选择", type: "warning" });
            return;
          }
          // 没有customerStoreId的用户,如果你编辑也是走创建用户接口
          if (this.option == "add" || this.formData.customerStoreId == "" || this.channel == 'app') {
            //新增
            updateEnabled(this.formData)
              ...
              .catch((err) => console.log(err));
          } else if (this.option == "transform") { //转为客户 
            clueSwitch({ ...this.formData, clueId: this.clueId })
              .then((res) => {
                if (res.resp_code == 0) {
                  this.formData.source == ""
                    ? (this.formData.source = 0)
    ...
    clueDetail () {
      clueDetail({ id: this.detailId })
        .then(res => {
          this.formData.storeName = res.data.name
          this.formData.linkman = res.data.linkman;
          this.formData.telephone = res.data.telephone;
          this.formData.companyArea = res.data.areaName  //地址回显
          this.formData.areaId = res.data.areaId || '0000'//地址id回显
          this.formData.address = res.data.address  //地址回显
